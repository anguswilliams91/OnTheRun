...
\end{equation}
%
so that a star travelling at speed $\vesc(r)$ at radius $r$ will be stationary at radius $r_\text{max}$.
$r_\text{max}$ is the radius beyond which stars are no longer bound to the Milky Way.
...
Indeed, P14 showed that setting $r_\text{max}=3\,R_\text{vir}$ instead of $r_\text{max}=\infty$ introduced a 20\% difference in the inferred virial mass.
By {\it mapping} the escape speed, we no longer need to make contact with $r_\text{max}$.
...
\end{equation}
%
so that the dependence on $r_\text{max}$ is removed.
We infer $\vesc$ {\it and its derivative} from the data, whereas previous authors measured $\vesc(R_0)$ only, so that they had to interact directly with Equation (\ref{eq:vesc_def}) and hence $r_\text{max}$. 
