...
By {\it mapping} the escape speed, we do not need to make contact with $r_\text{max}$ in order to infer the enclosed mass.
...
\end{equation}
%
We infer $\vesc$ {\it and} $\dfrac{\mathrm{d}\vesc}{\mathrm{d}r}$ from the data, so that the mass enclosed at radius $r$ can be inferred without considering the Galactic mass profile exterior to $r$. 
The same reasoning applies to the rotation curve because $v_c^2(r) = GM(r)/r$.
Consequently, the mass and circular speed profiles for the Galaxy can be inferred over the range for which we have mapped $\vesc(r)$: $6 \lesssim r/\kpc \lesssim 50$.
