...
The gradient in the escape speed with radius suggests that the total mass contained within $50\kpc$ is $30^{+7}_{-5}\times10^{10}\Msun$, implying a relatively light dark halo for the Milky Way. 
...
In a spherical potential, the two quantities are related through the energy equation
%
\begin{equation}
...
\end{equation}
%
so that a star travelling at speed $\vesc(r)$ at r will have zero velocity at $r=r_\text{max}$, the radius at which tracers are considered unbound from the Milky Way.
...
An obvious yet important assumption of this model is that stellar velocities really do extend to $\vesc$, instead of at some smaller value.
...
As line of sight velocities are measured far more precisely than transverse velocities, it is useful to marginalise the above model over proper motion. 
Reasoning that
%
\begin{equation}
  p(v)\propto\begin{cases}
...
The spherical model has a flat rotation curve of amplitude $v_0$ in the inner parts, which then starts to decline as a power-law $\gamma/2$ around a scale radius $r_s$
%
\begin{equation}
...
The priors we use for $\beta$ and $\vesc(R_0)$ are the same as in Equations (\ref{eq:powerlawprior}) and (\ref{eq:jeffreys}). For $q$, we follow \citet{Bo16} and use a prior
%
\begin{equation}
...
S07 and P14 both converted their measurements of the escape speed into a virial mass for the Galaxy, although they had to make educated guesses for an appropriate value of $r_\text{max}$ in Equation (\ref{eq:vesc_def}), which could have introduced systematic biases into their mass estimates.
Fortunately, by mapping the escape speed, we remove this issue.
The mass enclosed at radius $r$ in a spherical Galaxy is
%
\begin{equation}
M(r) = \dfrac{r^2}{G}\dfrac{\mathrm{d}\Phi}{\mathrm{d}r}.
\end{equation}
%
Eliminating $\Phi(r)$ in place of $\vesc(r)$ via $\vesc(r) = \sqrt{-2\left[\Phi(r) - \Phi(r_\text{max})\right]}$, we find
%
\begin{equation}
M(r) = \dfrac{-r^2\,\vesc}{G}\dfrac{\mathrm{d}\vesc}{\mathrm{d}r},
\end{equation}
%
and the dependence on $r_\text{max}$ is removed.
The same reasoning applies to the rotation curve because $v_c^2(r) = GM(r)/r$.
Consequently, the mass and circular speed profiles for the Galaxy that we infer are reliable over the range for which we have mapped $\vesc(r)$: $6 \lesssim r/\kpc \lesssim 50$.
...
The red points in Figure \ref{fig:sats} are Bootes III \citep{Gr09}, Triangulum II \citep{La15} and Hercules \citep{Be07}, all of which are likely associated with the Milky Way and have large line of sight velocities. 
...
For the three Milky Way dwarfs, we would like to characterise their orbits through their pericentre radii $\rp$, apocentre radii $\ra$ and orbital eccentricities $\ecc$.
...
To sample this distribution, we draw a sample from Equation (\ref{eq:ppc}) that is the same size as the data at each draw from the posterior in our MCMC chain \citep{Ge13}, producing many replicated data sets.
